...
static void files_mouse(window_t *w, int lx, int ly, int buttons, int prev) {
    ...
    rect_t body = R(cl.x, cl.y + 39, cl.w, cl.h - 39);
    int sel = -1;
    for (int i = 0; i < 4; i++)
        if (rect_contains(files_item_rect(body, i), cl.x + lx, cl.y + ly)) sel = i;
    if (sel != st->selected) { st->selected = sel; compositor_add_damage(cl); }
}

static void free_data(window_t *w) { if (w->data) { kfree(w->data); w->data = 0; } }

/* ════════════════════════════════════════════════════════════════════════════
   Notepad  (a real text editor: insert, delete, arrows, home/end, scrolling, blinking caret)
   ════════════════════════════════════════════════════════════════════════════ */
#define NP_MAX 8192
#define NP_LINE_H 18
...
typedef struct {
    ...
    int  len, caret;
    int  scroll;               /* first visible line */
    int  want_col;             /* remembered column for up/down */
    bool caret_on;
} notepad_t;
...
static void np_insert(notepad_t *n, char c) {
    if (n->len >= NP_MAX) return;
    memmove(n->buf + n->caret + 1, n->buf + n->caret, (size_t)(n->len - n->caret));
    n->buf[n->caret++] = c;
    n->buf[++n->len] = '\0';
}

static void np_delete_at(notepad_t *n, int pos) {
    if (pos < 0 || pos >= n->len) return;
    memmove(n->buf + pos, n->buf + pos + 1, (size_t)(n->len - pos - 1));
    n->buf[--n->len] = '\0';
}
...
static void notepad_key(window_t *w, uint16_t key, uint8_t mods) {
    ...
    int line, col;
    if (mods & (MOD_CTRL | MOD_ALT)) return;
    if (key >= 32 && key < 127) np_insert(n, (char)key);
    else if (key == '\n') np_insert(n, '\n');
    else if (key == '\t') { np_insert(n, ' '); np_insert(n, ' '); np_insert(n, ' '); np_insert(n, ' '); }
    else if (key == KEY_BACKSPACE) { if (n->caret > 0) { np_delete_at(n, n->caret - 1); n->caret--; } }
    else if (key == KEY_DELETE) np_delete_at(n, n->caret);
    else if (key == KEY_LEFT) { if (n->caret > 0) n->caret--; }
    else if (key == KEY_RIGHT) { if (n->caret < n->len) n->caret++; }
    else if (key == KEY_HOME || key == KEY_END) {
        np_line_col(n, n->caret, &line, &col);
        int s = np_line_start(n, line);
        n->caret = (key == KEY_HOME) ? s : s + np_line_len(n, s);
        ...
        if (s >= 0) {
            int ll = np_line_len(n, s);
            n->caret = s + (n->want_col < ll ? n->want_col : ll);
        }
        n->caret_on = true;
        np_scroll_into_view(n, wm_client_rect(w));
        compositor_add_damage(wm_client_rect(w));
        return;
    } else return;
...
static void notepad_mouse(window_t *w, int lx, int ly, int buttons, int prev) {
    if (!((buttons & 1) && !(prev & 1))) return;
    notepad_t *n = (notepad_t *)w->data;
    rect_t c = wm_client_rect(w);
    rect_t t = np_text_rect(c);
    int y = c.y + ly;
    if (y < t.y) return;
    int line = n->scroll + (y - t.y - 8) / NP_LINE_H;
    if (y - t.y < 8) line = n->scroll;
    int s = np_line_start(n, line);
    if (s < 0) { int last, dummy; np_line_col(n, n->len, &last, &dummy); s = np_line_start(n, last); }
    int col = (c.x + lx - t.x - 10 + 4) / 8;
    if (col < 0) col = 0;
    int ll = np_line_len(n, s);
    n->caret = s + (col < ll ? col : ll);
    n->want_col = -1;
    n->caret_on = true;
    compositor_add_damage(c);
}

static const char *np_initial =
    "BornomalaOS Notepad\n\n"
    "This window is drawn by the kernel's own window manager.\n"
    "Click here and type: text, Backspace, Delete, arrow keys,\n"
...
static void calc_click(window_t *w, int lx, int ly, int buttons, int prev) {
    calc_t *st = (calc_t *)w->data;
    rect_t c = wm_client_rect(w);
    int x = c.x + lx, y = c.y + ly;
    bool down = (buttons & 1) && !(prev & 1);
    bool up = !(buttons & 1) && (prev & 1);
    if (down) {
        for (int i = 0; i < CALC_BTNS; i++)
            if (rect_contains(calc_button_rect(c, i), x, y)) { st->pressed = i; compositor_add_damage(c); return; }
    } else if (up && st->pressed >= 0) {
        int i = st->pressed;
        st->pressed = -1;
        if (rect_contains(calc_button_rect(c, i), x, y)) calc_press(st, calc_keys[i]);
...
/* ════════════════════════════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════════════════════════════ */
window_t *apps_launch(app_id_t id, int sw, int sh, const char *username) {
    window_t *existing = wm_find_app(id);
    if (existing) { wm_activate(existing); return existing; }

    rect_t r; const char *title; icon_id_t icon; int minw = 300, minh = 200;
    switch (id) {
    case APP_COMPUTER: r = R(190, 80, 720, 455);  title = "Computer";   icon = ICON_COMPUTER; break;
    case APP_FILES:    r = R(235, 120, 690, 420); title = "Files";      icon = ICON_FILES;    break;
    case APP_NOTEPAD:  r = R(280, 135, 620, 390); title = "Notepad - Untitled"; icon = ICON_NOTEPAD; break;
    case APP_CALC:     r = R(350, 120, 340, 390); title = "Calculator"; icon = ICON_CALC;     break;
    default: return 0;
    }
    /* keep the window on small screens */
    if (r.x + r.w > sw - 10) r.x = sw - 10 - r.w;
    if (r.y + r.h > sh - 90) r.y = sh - 90 - r.h;
    if (r.x < 10) r.x = 10;
    if (r.y < 40) r.y = 40;
    ...
    window_t *w = wm_create(id, title, icon, r, minw, minh);
    if (!w) return 0;

    switch (id) {
    case APP_COMPUTER:
        w->paint = computer_paint;
        break;
    case APP_FILES: {
        files_state_t *st = (files_state_t *)kcalloc(1, sizeof(*st));
        if (!st) { wm_close(w); return 0; }
        st->selected = -1;
        w->data = st; w->paint = files_paint; w->on_mouse = files_mouse; w->on_close = free_data;
        break; }
    case APP_NOTEPAD: {
        notepad_t *n = (notepad_t *)kcalloc(1, sizeof(*n));
        if (!n) { wm_close(w); return 0; }
        k_strncpy(n->buf, np_initial, NP_MAX);
        n->len = (int)k_strlen(n->buf);
        n->caret = n->len; n->want_col = -1; n->caret_on = true;
        w->data = n; w->paint = notepad_paint; w->on_key = notepad_key; w->on_mouse = notepad_mouse; w->on_close = free_data;
        break; }
    case APP_CALC: {
        calc_t *cs = (calc_t *)kcalloc(1, sizeof(*cs));
        if (!cs) { wm_close(w); return 0; }
        calc_reset(cs);
        w->data = cs; w->paint = calc_paint; w->on_mouse = calc_click; w->on_key = calc_key; w->on_close = free_data;
        break; }
    default: break;
...
void apps_tick(uint64_t ticks) {
    window_t *w = wm_find_app(APP_NOTEPAD);
    if (!w || w->minimized || wm_active() != w) return;
    notepad_t *n = (notepad_t *)w->data;
    bool on = ((ticks / 50) & 1) == 0;                 /* 2 Hz blink at 100 Hz timer */
    if (on != n->caret_on) {
